...
<body <?php body_class(); ?>>
<?php wp_body_open(); ?>
<header class="absolute mx-[120px] inset-0 h-[200px] z-50 hidden xl:flex items-center justify-between">
...
<header class="container mx-auto mobile absolute inset-0 h-[150px] z-50 flex xl:hidden items-center justify-between" x-data="{ sidebarOpen: false }" @keydown.window.escape="sidebarOpen = false">
    <a href="<?php echo esc_url( home_url( '/' ) ); ?>" rel="home">
    ...
    <div class="fixed inset-0" x-show="sidebarOpen" style="display: none;">
        ...
        <div class="h-full ml-auto relative flex-1 flex flex-col max-w-full w-full pt-[100px] bg-aurora-blue-01" x-show="sidebarOpen" x-transition:enter="transition ease-in-out duration-300 transform" x-transition:enter-start="-translate-y-full" x-transition:enter-end="translate-y-0" x-transition:leave="transition ease-in-out duration-300 transform" x-transition:leave-start="translate-y-0" x-transition:leave-end="-translate-y-full">
